...
<script setup>
import { ref } from 'vue'
import { useRefundsStore } from '../stores/refunds.js'
import { useUserStore } from '../stores/user.js'
const useUser = useUserStore()

const useRefunds = useRefundsStore();
// const useUser = useUserStore()
...
async function getRefunds() {
    ...
    useRefunds.createToken(useUser.token)
    const res = await useRefunds.getRefunds()
    rows.value = res
...
async function save() {
    ...
    if (bd.value == 0) {
        await useRefunds.addRefunds(data)
        clean()
        modal.value = false
    } else if (bd.value == 1) {
        await useRefunds.editRefunds(id, data)
        modal.value = false
        // clean()
    }
    getRefunds()
...
async function active(row) {
    loading.value = true;
    await useRefunds.activeRefunds(row._id)
    loading.value = false;
    getRefunds()
}
...
async function inactive(row) {
    loading.value = true;
    await useRefunds.inactiveRefunds(row._id)
    loading.value = false;
    getRefunds()
}
