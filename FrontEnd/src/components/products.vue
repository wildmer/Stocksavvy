...
async function getProducts() {
  ...
  useProducts.createToken(useUser.token)
  const res = await useProducts.getProducts()
  rows.value = res
...
async function save() {
  ...
  if (bd.value == 0) {
    await useProducts.addProducts(data)
    clean()
    modal.value = false
  } else if (bd.value == 1) {
    await useProducts.editProducts(id, data)
    modal.value = false
    // clean()
  }
  getProducts()
...
async function active(row) {
  loading.value = true;
  await useProducts.activeProducts(row._id)
  loading.value = false;
  getProducts()
}
...
async function inactive(row) {
  loading.value = true;
  await useProducts.inactiveProducts(row._id)
  loading.value = false;
  getProducts()
}
