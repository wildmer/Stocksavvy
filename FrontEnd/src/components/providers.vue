...
const useProviders = useProvidersStore();
// const useUser = useUserStore()
...
async function getProviders() {
  ...
  useProviders.createToken(useUser.token)
  const res = await useProviders.getProviders()
  rows.value = res
...
async function save() {
  ...
  if (bd.value == 0) {
    await useProviders.addProviders(data)
    clean()
    modal.value = false
  } else if (bd.value == 1) {
    await useProviders.editProviders(id, data)
    modal.value = false
    // clean()
  }
  getProviders()
...
async function active(row) {
  loading.value = true;
  await useProviders.activeProviders(row._id)
  loading.value = false;
  getProviders()
}
...
async function inactive(row) {
  loading.value = true;
  await useProviders.inactiveProviders(row._id)
  loading.value = false;
  getProviders()
}
