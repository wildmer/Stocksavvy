...
<script setup>
import { ref } from 'vue'
import { useOutsStore } from '../stores/outs.js'
import { useUserStore } from '../stores/user.js'
const useUser = useUserStore()

const useOuts = useOutsStore();
// const useUser = useUserStore()
...
async function getOuts() {
  ...
  useOuts.createToken(useUser.token)
  const res = await useOuts.getOuts()
  rows.value = res
...
async function save() {
  ...
  if (bd.value == 0) {
    await useOuts.addOuts(data)
    clean()
    modal.value = false
  } else if (bd.value == 1) {
    await useOuts.editOuts(id, data)
    modal.value = false
    // clean()
  }
  getOuts()
...
async function active(row) {
  loading.value = true;
  await useOuts.activeOuts(row._id)
  loading.value = false;
  getOuts()
}
...
async function inactive(row) {
  loading.value = true;
  await useOuts.inactiveOuts(row._id)
  loading.value = false;
  getOuts()
}
