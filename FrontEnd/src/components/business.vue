<template>
  <div class="q-pa-md">
    ...
    <q-table title="Empresas" :rows="rows" :columns="columns" row-key="name">
  ...
  <q-btn color="white" text-color="black" label="Nueva Empresa" @click="add()"
    ...
    <q-card class="bg-teal text-dark" style="width: 500px; max-width: 80vw">
      <q-card-section style="background-color: rgb(14, 224, 14)">
        <div align="center" class="text-h6">
          {{ bd === 0 ? "Guardar empresa" : "Editar empresa" }}
        </div>
      </q-card-section>
...
async function getBusiness() {
  ...
  useBusiness.createToken(useUser.token)
  const res = await useBusiness.getBusiness()
  rows.value = res
...
async function save() {
  ...
  if (bd.value == 0) {
    await useBusiness.addBusiness(data)
    clean()
    modal.value = false
  } else if (bd.value == 1) {
    await useBusiness.editBusiness(id, data)
    modal.value = false
    // clean()
  }
  getBusiness()
...
async function active(row) {
  loading.value = true;
  await useBusiness.activeBusiness(row._id)
  loading.value = false;
  getBusiness()
}
...
async function inactive(row) {
  loading.value = true;
  await useBusiness.inactiveBusiness(row._id)
  loading.value = false;
  getBusiness()
}
